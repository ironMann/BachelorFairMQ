...
class scheduler : public FairMQDevice
{
  ...
  //for EPN SCHEDULER DEALING
    ...
    const unsigned amountEPNs; //amount of EPNs that are in the array the Scheduler needs to send to the FLPs
  ...
  //for SCHEDULER FLP DEALING
    ...
    std::thread senderThread(int* arr);
    ...
    void printfreeSlots(int arr[], int length); //function to print the array that will be sent to all FLPs
    void sender(int* arr);
