...
{
    auto &myRecvChan = GetChannel("epnsched");
    ...
    cout<<" interval FLPS * intMs: "<< intervalFLPs*intMs << endl;
    cout << "localkey: " << localkey << endl;
    cout <<"key for generatingarray: " << keyForGeneratingArray << endl;
    cout << "keyForGeneratingArray + intervalFLPs*intMs: "<< (keyForGeneratingArray + (intervalFLPs*intMs)) << endl;
    bool c = (localkey>=(keyForGeneratingArray + intervalFLPs*intMs));
    cout << "c: " << c << endl;
...
int* scheduler::generateArray(){
    int* temporaryStorage= new int[1]; //pointer to array of amount of EPNs which we need to store the memory size of the valid EPNs
    int* desFLPsPointer = new int[1]; // pointer to the array which the function will give back and contains the IDs of the EPNs that the FLPs need to send their subtimeframes to, beginning from the lowest ID and then in ascending order...
...
void scheduler::sender(int* arr){
    ...
    SchedFLPTest Msg;
    Msg.IdForFirst= arr[0];
    /*
    Msg.IdForSecondEpn= arr[1];
    Msg.IdForThirdEpn= arr[2];
    Msg.IdForFourthEpn= arr[3];
    */

    FairMQMessagePtr message = mySendingChan.NewMessage(sizeof(SchedFLPTest));
    std::memcpy(message->GetData(), &Msg, sizeof(SchedFLPTest));
    mySendingChan.Send(message);

    LOG(INFO)<<"sent Id for First Epn: " << Msg.IdForFirst << endl;
