  ...
  for(int i=0; i < numEPNS; i++){
    ...
    uint64_t localkey= getHistKey();
     if(localkey>=(keyForGeneratingArray + intervalFLPs*intMs)){
        ...
        std::thread t1= senderThread(arrayForFlps, &numFLPS);
        t1.detach();
        }
...
thread scheduler::senderThread(int* arr, uint64_t* num){
    return std::thread([=]{
    this->sender(arr, num);
  });
}
...
void scheduler::sender(int* arr, uint64_t* num){
    ...
    LOG(INFO)<<"sent Id for First Epn: " << Msg.IdForThirdEpn << " "<<i<<" !"<< endl;
    }
